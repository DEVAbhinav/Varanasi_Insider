...
export default function CTASection({ 
  ...
  const variants = {
    default: {
      container: "py-8 px-6 my-8",
      maxWidth: "max-w-4xl",
      titleSize: "text-2xl",
      layout: "flex-col md:flex-row"
    },
    compact: {
      container: "py-6 px-4 my-6",
      maxWidth: "max-w-3xl",
      titleSize: "text-xl",
      layout: "flex-col md:flex-row"
    },
  ...
  return (
    <section className={`bg-gradient-to-r from-yellow-500 to-yellow-600 ${style.container}`}>
      <div className={`${style.maxWidth} mx-auto flex ${style.layout} items-center justify-between gap-4`}>
        <div className="text-white text-center md:text-left">
          <h3 className={`${style.titleSize} font-bold mb-1`}>{title}</h3>
          <p className="text-yellow-50">{subtitle}</p>
        </div>
