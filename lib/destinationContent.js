...
import { markdownToHtml, demoteContentHeadings } from './markdown.js';
import { extractItineraryFromMarkdown } from './itinerary.js';
import { getBreadcrumbConfig } from './categories.js';
import { sanitizeJsonLdData } from './jsonLdSanitizer.js';
...
const shouldIncludeCanonicalRoute = (routePath, canonical, baseUrl = DEFAULT_SITE_BASE) => {
  if (!canonical) return true;
  const normalizedCanonical = ensureAbsoluteUrl(canonical, baseUrl)?.replace(/\/+$/, '') || null;
  const normalizedCurrent = ensureAbsoluteUrl(routePath, baseUrl)?.replace(/\/+$/, '') || null;
...
export const getDestinationEntries = (category, lang = 'en') => {
  ...
  destinations.forEach((destination) => {
    ...
    files.forEach((file) => {
      ...
      try {
        frontmatter = normalizeContactData(matter(fs.readFileSync(filePath, 'utf8')).data || {});
      } catch (err) {
        frontmatter = {};
      }

      const localizedPath = `/${lang}/city/${destination}/${category}/${slug}`;
      if (!shouldIncludeCanonicalRoute(localizedPath, frontmatter.canonical, DEFAULT_SITE_BASE)) {
        return;
      }
...
export const getDestinationEntry = async (lang, category, destination, slug) => {
  ...
  const frontmatter = normalizeContactData(data || {});
  const canonicalUrl = ensureAbsoluteUrl(frontmatter?.canonical, baseUrl) || `${baseUrl}${localizedPath}`;
  ...
  // DestinationContentPage renders its own <h1> header, so demote the body's
  // leading markdown H1 to H2 to avoid a duplicate (multiple-H1) on the page.
  const contentHtmlFull = demoteContentHeadings(await markdownToHtml(content || ''));
  ...
  const contentHtmlBefore = demoteContentHeadings(await markdownToHtml(beforeMarkdown || ''));
  const contentHtmlAfter = range ? demoteContentHeadings(await markdownToHtml(afterMarkdown || '')) : null;
  ...
  if (fs.existsSync(jsonPath)) {
    try {
      const parsed = normalizeContactData(JSON.parse(fs.readFileSync(jsonPath, 'utf8')));
      if (parsed['@graph']) {
  ...
  if (category === DESTINATION_CATEGORIES.TOUR_PACKAGES) {
    const productSchema = generateProductSchema({
      ...
      productGroup: frontmatter.productGroup,
  ...
  if (category === DESTINATION_CATEGORIES.TAXI) {
    ...
    // Google shows review stars for Product (not Service), so carry the rating on
    // a Product node. Only add one if the graph doesn't already have a Product
    // (e.g. from a manual JSON file) to avoid duplicate rating nodes.
    if ((frontmatter.aggregateRating?.ratingValue != null) || frontmatter.productGroup) {
      const hasProduct = jsonLd['@graph'].some((node) => {
        const t = node && node['@type'];
        return t === 'Product' || (Array.isArray(t) && t.includes('Product'));
      });
      if (!hasProduct) {
        const ratingProduct = generateProductSchema({
          title: pageTitle,
          description: frontmatter.description,
          url: canonicalUrl,
          image: frontmatter.featuredImage ? ensureAbsoluteUrl(frontmatter.featuredImage, baseUrl) : undefined,
          offers: frontmatter.offers,
          aggregateRating: frontmatter.aggregateRating,
          review: frontmatter.reviews,
          productGroup: frontmatter.productGroup,
          brand: frontmatter.brand,
          location: frontmatter.location
        });
        jsonLd['@graph'].push(ratingProduct);
      }
    }
  }
  ...
  const sanitizedJsonLd = sanitizeJsonLdData(jsonLd);
  ...
  return {
    ...
    contentHtmlAfter,
    itinerarySections,
    jsonLd: sanitizedJsonLd, // Merge legacy + generated schema and drop malformed nodes
    ...frontmatter,
    itinerary,
    breadcrumbs,
