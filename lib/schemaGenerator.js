...
import { getGroupRating } from '@/lib/ratingGenerator';
...
export function generateServiceSchema(data) {
    ...
    const schema = {
        ...
        provider: {
            '@type': 'LocalBusiness',
            '@id': ORGANIZATION_ID,
            name: provider?.name || BUSINESS.brandName,
    ...
    // NOTE: aggregateRating is intentionally NOT emitted on the Service node.
    // Google does not allow review snippets on the Service type ("Invalid object
    // type for field") — the rating is carried on a Product node instead.
...
export function generateArticleSchema(data) {
    ...
    const schema = {
        ...
        author: {
            '@type': 'Person',
            name: authorName || `${BUSINESS.brandName} Team`
        },
        publisher: {
            '@type': 'Organization',
            name: publisherName || BUSINESS.brandName,
            logo: {
                '@type': 'ImageObject',
...
export function generateProductSchema(data) {
    const {
        ...
        offers,
        aggregateRating,
        // Optional: one of PRODUCT_GROUPS from lib/ratingGenerator. When
        // aggregateRating isn't explicitly provided, the rating is derived
        // from this group so the same vehicle/service type always shows the
        // same rating + review count across every page it appears on.
        productGroup,
        brand,
        review,
        location // Add location
    ...
    if (aggregateRating) {
        ...
        schema.aggregateRating = getGroupRating(productGroup);
    }
...
export function generateEventSchema(data) {
    ...
    if (organizer) {
        schema.organizer = {
            '@type': 'Organization',
            name: organizer.name || BUSINESS.brandName,
