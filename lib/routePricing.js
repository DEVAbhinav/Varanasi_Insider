 ...
 * @param {object} [args.fixedFare]        - optional fixed fare mapping by vehicle id: { [vehicleId]: { ow, rt } }
 * @returns {null | {
...
export function estimateRoute({
  ...
  let calculatedFare = round(fareKm + toll + driver);

  // Minimum floor check (e.g. Kathmandu starting from ₹27,990 minimum; extra km/days scale over it)
  ...
  if (fareMap && fareMap[v.id]) {
    ...
    if (Number.isFinite(minThreshold)) {
      floorAmount = minThreshold;
      if (calculatedFare < minThreshold) {
        calculatedFare = minThreshold;
  ...
  const lines = [
    { label: `${v.name} — ${isRound ? 'round trip' : 'one way'} (₹${perKm}/km × ${billableKm} km${minFloorApplied ? `, min floor ₹${floorAmount.toLocaleString('en-IN')}` : isFixedFloor ? ', min-km' : ''})`, amount: round(fareKm) },
  ...
  if (minFloorApplied) {
    const diff = calculatedFare - round(fareKm + toll + driver);
    if (diff > 0) {
      lines.push({ label: `Route minimum floor (extra km/days billed over this)`, amount: diff });
    }
  }
  ...
  return {
    ...
    minBillKm: routeMin,
    isFixedFloor: isFixedFloor || minFloorApplied,
    stateTaxNote: 'State tax extra as applicable (e.g. Bihar/MP border routes).',
    assumptions: isRound
      ? `Round trip billed on max(2×${dist} km, ${routeMin} km min) = ${billableKm} km, plus toll estimate${n ? ` and ${n} night halt` : ''}${minFloorApplied ? ` (minimum route floor ₹${floorAmount.toLocaleString('en-IN')})` : ''}. State tax extra where applicable.`
      : `One-way${isFixedFloor ? ' fixed local fare' : ''}, billed on max(${dist} km, ${floor} km min) = ${billableKm} km, plus toll estimate${n ? ` and ${n} night halt` : ''}${minFloorApplied ? ` (minimum route floor ₹${floorAmount.toLocaleString('en-IN')})` : ''}. State tax extra where applicable.`,
  };
}

/** Estimate all vehicles for a route — returns an array sorted cheapest-first. */
export function estimateAllVehicles({ distanceKm, tripType = 'round-trip', days = 1, nights = 0, minBillKm, fixedFare, minFare } = {}) {
  return vehicles
    .map((v) => estimateRoute({ vehicleId: v.id, distanceKm, tripType, days, nights, minBillKm, fixedFare, minFare }))
    .filter(Boolean)
