...
export async function loadMarkdownContent(lang, relativePath) {
  const normalizedRelativePath = `${normalizeSlugInput(relativePath)}.md`;
  const filePath = path.join(getLangRoot(lang), normalizedRelativePath);
...
function normalizeFrontmatterSlug(slug = '') {
  return normalizeSlugInput(slug).toLowerCase();
}

function resolveRoutePathFromFilePath(lang, fullPath, frontmatter = {}) {
  const langRoot = getLangRoot(lang);
  ...
  const filenameSlug = normalizeFrontmatterSlug(parts[parts.length - 1]);
  const resolvedSlug = frontmatterSlug || filenameSlug;

  if (parts.length === 1) {
    return `/${lang}/${resolvedSlug}`;
  }

  if (parts[0] === 'services' || parts[0] === 'landing' || parts[0] === 'guides') {
    return `/${lang}/services/${resolvedSlug}`;
  }

  if (parts[0] === 'packages') {
    return `/${lang}/packages/${resolvedSlug}`;
  }

  if (parts[0] === 'bus') {
    return `/${lang}/bus/${resolvedSlug}`;
  }

  if (parts[0] === 'destinations' && parts.length >= 4) {
    const [, destination, category] = parts;
    return `/${lang}/city/${destination}/${category}/${resolvedSlug}`;
  }
  ...
  const allPostsData = filePaths
    .map((fullPath) => {
      try {
        ...
        const fm = normalizeContactData(matterResult.data || {});
        const routePath = resolveRoutePathFromFilePath(lang, fullPath, fm);
...
export function getAllPostPaths() {
  ...
  languages.forEach((lang) => {
    ...
    filePaths.forEach((fullPath) => {
      let fm = {};
      try {
        fm = normalizeContactData(matter(fs.readFileSync(fullPath, 'utf8')).data || {});
      } catch (err) {
        fm = {};
      }
...
export async function getPostData(lang, slug) {
  ...
  try {
    ...
    const autoItinerary = normalizeContactData(extracted?.itinerary || null);
...
export async function getJsonLdData(lang, slug) {
  const { generateArticleSchema, generateBreadcrumbSchema, generateFAQSchema, generateProductSchema } = await import('./schemaGenerator');
  ...
  if (mdPath) {
    try {
      ...
      postData = normalizeContactData(matter(fileContents).data || {});
    } catch (err) {
  ...
  try {
    ...
    jsonLdData = normalizeContactData(JSON.parse(fileContents));
  } catch (err) {
    if (err.code === 'ENOENT') {
  ...
  // --- AggregateRating from frontmatter (drives SERP review-star rich results) ---
  // Google shows review stars for Product (not Article/Service), so attach the
  // rating to an existing Product node or create one. Skipped if a rating already
  // exists in the graph (e.g. supplied by a manual JSON file) to avoid duplicates.
  const fmRating = postData.aggregateRating;
  if (fmRating && fmRating.ratingValue != null) {
    const graph = jsonLdData['@graph'];
    const hasRating = graph.some((node) => {
      if (!node || typeof node !== 'object') return false;
      const type = node['@type'];
      const isRatingNode = Array.isArray(type) ? type.includes('AggregateRating') : type === 'AggregateRating';
      return isRatingNode || !!node.aggregateRating;
    });
    if (!hasRating) {
      const ratingObj = {
        '@type': 'AggregateRating',
        ratingValue: fmRating.ratingValue,
        reviewCount: fmRating.reviewCount || fmRating.ratingCount,
        bestRating: '5',
        worstRating: '1'
      };
      const productNode = graph.find((node) => {
        const type = node && node['@type'];
        return type === 'Product' || (Array.isArray(type) && type.includes('Product'));
      });
      if (productNode) {
        productNode.aggregateRating = ratingObj;
      } else {
        const siteUrl = 'https://www.kashitaxi.in';
        const pageUrl = `${siteUrl}/${lang}/${slug}`;
...
// Footer is the only consumer of this data and renders at most `perLangLimit`
// posts per language. Returning only the displayed posts (plus real per-language
// totals for the "View all N posts" link) keeps each page's serialized props tiny
// instead of embedding metadata for every post on the site into every page.
export function getAllPostsMeta({ perLangLimit = 12 } = {}) {
  let languages = [];
  try {
  ...
  languages.forEach((lang) => {
    ...
    counts[lang] = postsInLang.length;
    postsInLang.slice(0, perLangLimit).forEach((p) => {
      posts.push({
        lang,
        slug: p.slug,
