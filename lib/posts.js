...
function findMarkdownBySlug(lang, slug) {
  ...
  // Handle slugs that may include folder prefix like "services/slug-name"
  const target = path.basename(slug).toLowerCase();
  return files.find((filePath) => path.basename(filePath, '.md').toLowerCase() === target) || null;
}
