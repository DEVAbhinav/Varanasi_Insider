...
import { SYSTEM_PROMPT, PERSONA_DETECTION_PROMPT, TOPIC_EXTRACTION_PROMPT } from '@/lib/assistant/prompts/system-prompt';
...
interface LLMResponse {
    content: string;
    llmUsed: 'Gemini 1.5 Flash' | 'GPT-4o Mini';
...
const config = {
    ...
    primaryLLM: process.env.PRIMARY_LLM || 'gemini-1.5-flash',
...
export async function generateResponse(
    ...
    let response: string;
    let llmUsed: 'Gemini 1.5 Flash' | 'GPT-4o Mini';
...
async function callGPT(
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
            model: config.fallbackLLM || 'gpt-4o-mini',
